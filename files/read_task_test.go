...
`
	...
	err := os.WriteFile(taskPath, []byte(invalidContent), 0644)
	require.NoError(t, err)
...
`
	...
	err := os.WriteFile(taskPath, []byte(incompleteContent), 0644)
	require.NoError(t, err)
...
`
	...
	err := os.WriteFile(taskPath, []byte(minimalContent), 0644)
	require.NoError(t, err)
...
`
	...
	err := os.WriteFile(taskPath, []byte(specialContent), 0644)
	require.NoError(t, err)
...
`
	...
	err := os.WriteFile(taskPath, []byte(complexContent), 0644)
	require.NoError(t, err)
...
func TestReadTask_FilePermissionError(t *testing.T) {
	...
	err := os.Mkdir(readOnlyDir, 0755) // Normal directory first
	require.NoError(t, err)
...
`
	...
	err = os.Chmod(readOnlyDir, 0500) // Read-only, no execute permissions
	require.NoError(t, err)
